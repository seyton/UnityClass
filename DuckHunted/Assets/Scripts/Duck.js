...
function Start () {
	...
	// Animation Layers
	model.GetComponent.<Animation>()["Quack"].layer = 1;
...
function Update () {
	...
	if (isDead){
		...
		if (hasPlayedThud == false)
		{
			...
			if (Physics.Raycast (groundRay, groundRayHit, 1))
			{
				...
				GetComponent.<AudioSource>().clip = thudClip;
	...
	// Be a duck and fly around
		...
		if (quackTime < 0){
			transform.FindChild("Model").GetComponent.<Animation>().Play("Quack");
			GetComponent.<AudioSource>().Play(0);
			quackTime += Random.Range(minQuack, maxQuack);
		}
		else
		{
			transform.FindChild("Model").GetComponent.<Animation>().Play("Flap"); // Flap those wings
...
function Death() 
{
	...
	GetComponent.<AudioSource>().Play(0.25);
	GetComponent.<AudioSource>().pitch = 1;
	
	// Show score value		
	var totalScoreValue : int = scoreValue + (scoreValue * (player.GetComponent(Player).combo * player.GetComponent(Player).comboFactor));
	
	var textScoreMesh = Instantiate(textMesh, transform.position, Camera.main.transform.rotation);
