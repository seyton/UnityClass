...
function Update () 
{
	...
	if (!isGameOver)
	{
		...
		// Sprint
		if (Input.GetButtonDown("Sprint") && !gunTransform.FindChild("Model").GetComponent.<Animation>()["Reload"].enabled)
		{
			sprinting = true;
			...
			gunTransform.FindChild("Model").GetComponent.<Animation>().CrossFade("Run", 0.25);
		...
		{
			if (Input.GetButtonDown("Fire1") && !gunTransform.FindChild("Model").GetComponent.<Animation>()["Fire"].enabled && !gunTransform.FindChild("Model").GetComponent.<Animation>()["Reload"].enabled)
			{
				if (bullets > 0)
				{
					gunTransform.FindChild("Model").GetComponent.<Animation>().Play("Fire"); // Play firing animation, which in turns activates muzzle flash and gun sound
					...
					var fireRay = new Ray(Camera.main.transform.position, fireRayDir);
					...
					gunTransform.FindChild("Model").GetComponent.<Animation>().CrossFade("Reload", 0.25); // Plays reload animation, which in turns triggers the reload
			}
			...
			if (gunZoom == true)
			{
				...
				Camera.main.fov = Mathf.Lerp(Camera.main.fov, 30, Time.deltaTime*gunZoomSpeed);
			...
			else if (gunZoom == false)
			{
				...
				Camera.main.fov = Mathf.Lerp(Camera.main.fov, 60, Time.deltaTime*gunZoomSpeed);
		...
		/* Animation */
		if (!gunTransform.FindChild("Model").GetComponent.<Animation>()["Fire"].enabled && 
			!gunTransform.FindChild("Model").GetComponent.<Animation>()["Reload"].enabled && 
			!sprinting) // If no animation is playing, play Idle
		{
				gunTransform.FindChild("Model").GetComponent.<Animation>().CrossFade("Idle", 0.5);
		}
